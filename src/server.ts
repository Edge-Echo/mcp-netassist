...
function safe<T>(fn: () => Promise<T>, render: (value: T) => string) {
  return async () => {
    try {
      ...
      return { content: [{ type: 'text' as const, text: `netassist error: ${message}` }], isError: true }
    }
  }
}
...
server.tool(
  ...
  { ports: z.array(z.number().int()).optional().describe(`Ports to probe (default ${DEFAULT_PROXY_PORTS.join(', ')})`) },
  async ({ ports }) => {
    try {
