...
import {
  checkGithub, checkHosts, checkProxyPorts, checkProxyStatus, checkTunAdapters,
  DEFAULT_PROXY_PORTS, type GithubStatus, type HostsCheck, type PortProbe,
  type ProxyStatus, type TunCheck,
} from './checks.js'
...
export async function runDoctor(opts: { timeoutMs?: number; extraPorts?: number[] } = {}): Promise<DoctorReport> {
  ...
  if (github.reachable && (github.httpStatus ?? 0) > 0) {
    ...
    suggestions.push(
      'TCP works but TLS does not: usually the proxy does not cover github.com, or SNI is being interfered with. ' +
  ...
  if (hosts.hasGithub) {
    ...
    if (proxy.enabled || tun.detected) {
      suggestions.push(
        'Pinned GitHub IPs in the hosts file bypass your proxy and can go stale — back them up and remove them while a proxy is active.',
      )
    }
  } else {
